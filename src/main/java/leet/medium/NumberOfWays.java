...
//    space optimized combinatorics; time: O(n), space: O(1)
    public static int numberOfWays(String corridor) {
        int MOD = (int) 1e9 + 7;
        int seats = 0;
        Integer lastPairIndex = null;
        long count = 1;
        for(int index = 0 ; index < corridor.length() ; index++) {
            if(corridor.charAt(index) == 'S') {
                ...
                if (seats == 2) {
                    lastPairIndex = index;
                    seats = 0;
                } else if (seats == 1 && lastPairIndex != null) {
                    count *= (index - lastPairIndex);
                    count %= MOD;
                }
            }
        }
        if(seats == 1 || lastPairIndex == null) {
            return 0;
        }
        return (int) count;
    }
